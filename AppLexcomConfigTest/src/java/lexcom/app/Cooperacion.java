...
public class Cooperacion implements Serializable {
    ...
    private Corporacion_List selectedCooperacion;
    private List<Corporacion_List> lst_cooperacion;
    ...
    public void init() {
        try {
            ...
            lst_cooperacion = new ArrayList<>();
            for(Integer i = 1; i < filas; i++) {
                Corporacion_List cor = new Corporacion_List(
                        Integer.parseInt(vector_result[i][0]),
                        vector_result[i][1],
                        vector_result[i][2]);
                lst_cooperacion.add(cor);
            }
        } catch (Exception ex) {
    ...
    public void carga_info_modificar() {
        if(this.selectedCooperacion != null) {
            String cadenasql = "select "
                    + "c.nombre, "
                    + "c.descripcion "
                    + "from "
                    + "cooperacion c "
                    + "where "
                    + "c.cooperacion = " + this.selectedCooperacion.getCooperacion();
            Servicio servicio = new Servicio();
            java.util.List<lexcom.ws.StringArray> resultado = servicio.reporte(cadenasql, this.ambiente);
            Integer filas = resultado.size();
            Integer columnas = resultado.get(0).getItem().size();
            ...
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Mensaje del sistema...", "Dede seleccionar una corporación del listado."));
        }
    }
    ...
    private void insertar() {
        ...
        try {
            if (!this.nombre_d.equals("")) {
                ...
                resultado = servicio.corporacionInsertar(
                        id_usuario,
                        this.nombre_d,
                        this.descripcion_d, 
                        this.ambiente);
                this.constructor();
                RequestContext.getCurrentInstance().execute("PF('dtblWidgetCor').clearFilters();");

                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Mensaje del sistema...", resultado));
            } else {
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Mensaje del sistema...", "Debe ingresar el nombre de la corporación."));
            }
        } catch (Exception ex) {
    ...
    private void modificar() {
        ...
        try {
            if (!this.nombre_d.equals("")) {
                ...
                Servicio servicio = new Servicio();
                Integer id_usuario = driver.getInt("select u.usuario from usuario u where u.nombre = '" + this.usuario + "'", this.ambiente);
                resultado = servicio.corporacionModificar(
                        id_usuario,
                        this.selectedCooperacion.getCooperacion(),
                        this.nombre_d,
                        this.descripcion_d, 
                        this.ambiente);
                this.constructor();
    ...
    private void eliminar() {
        ...
        try {
            ...
            Servicio servicio = new Servicio();
            Integer id_usuario = driver.getInt("select u.usuario from usuario u where u.nombre = '" + this.usuario + "'", this.ambiente);
            resultado = servicio.corporacionEliminar(
                    id_usuario,
                    this.selectedCooperacion.getCooperacion(), 
                    this.ambiente);
            this.constructor();
    ...
    private void activar() {
        ...
        try {
            ...
            Servicio servicio = new Servicio();
            Integer id_usuario = driver.getInt("select u.usuario from usuario u where u.nombre = '" + this.usuario + "'", this.ambiente);
            resultado = servicio.corporacionActivar(
                    id_usuario,
                    this.selectedCooperacion.getCooperacion(), 
                    this.ambiente);
            this.constructor();
    ...
    public Corporacion_List getSelectedCooperacion() {
        return selectedCooperacion;
    }

    public void setSelectedCooperacion(Corporacion_List selectedCooperacion) {
        this.selectedCooperacion = selectedCooperacion;
    }

    public List<Corporacion_List> getLst_cooperacion() {
        return lst_cooperacion;
    }

    public void setLst_cooperacion(List<Corporacion_List> lst_cooperacion) {
        this.lst_cooperacion = lst_cooperacion;
    }
