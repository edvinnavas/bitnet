...
public class Antiguedad implements Serializable {
    ...
    private Antiguedad_List selectedAntiguedad;
    private List<Antiguedad_List> lst_antiguedad;
    ...
    public void init() {
        try {
            ...
            lst_antiguedad = new ArrayList<>();
            for(Integer i = 1; i < filas; i++) {
                Antiguedad_List gar = new Antiguedad_List(
                        Integer.parseInt(vector_result[i][0]),
                        vector_result[i][1],
                        vector_result[i][2]);
                lst_antiguedad.add(gar);
            }
        } catch (Exception ex) {
    ...
    public void carga_info_modificar() {
        if(this.selectedAntiguedad != null) {
            String cadenasql = "select "
                    + "g.nombre, "
                    + "g.descripcion "
                    + "from "
                    + "antiguedad g "
                    + "where "
                    + "g.antiguedad = " + this.selectedAntiguedad.getAntiguedad();
            Servicio servicio = new Servicio();
            java.util.List<lexcom.ws.StringArray> resultado = servicio.reporte(cadenasql, this.ambiente);
            Integer filas = resultado.size();
            Integer columnas = resultado.get(0).getItem().size();
            ...
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Mensaje del sistema...", "Dede seleccionar una antigüedad del listado."));
        }
    }
    ...
    private void insertar() {
        ...
        try {
            if (!this.nombre_d.equals("")) {
                ...
                resultado = servicio.antiguedadInsertar(
                        id_usuario,
                        this.nombre_d,
                        this.descripcion_d, 
                        this.ambiente);
                this.constructor();
                RequestContext.getCurrentInstance().execute("PF('dtblWidgetAnt').clearFilters();");

                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Mensaje del sistema...", resultado));
            } else {
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Mensaje del sistema...", "Debe ingresar el nombre de la antigüedad."));
            }
        } catch (Exception ex) {
    ...
    private void modificar() {
        ...
        try {
            if (!this.nombre_d.equals("")) {
                ...
                Servicio servicio = new Servicio();
                Integer id_usuario = driver.getInt("select u.usuario from usuario u where u.nombre = '" + this.usuario + "'", this.ambiente);
                resultado = servicio.antiguedadModificar(
                        id_usuario,
                        this.selectedAntiguedad.getAntiguedad(),
                        this.nombre_d,
                        this.descripcion_d, 
                        this.ambiente);
                this.constructor();
    ...
    private void eliminar() {
        ...
        try {
            ...
            Servicio servicio = new Servicio();
            Integer id_usuario = driver.getInt("select u.usuario from usuario u where u.nombre = '" + this.usuario + "'", this.ambiente);
            resultado = servicio.antiguedadEliminar(
                    id_usuario,
                    this.selectedAntiguedad.getAntiguedad(), 
                    this.ambiente);
            this.constructor();
    ...
    private void activar() {
        ...
        try {
            ...
            Servicio servicio = new Servicio();
            Integer id_usuario = driver.getInt("select u.usuario from usuario u where u.nombre = '" + this.usuario + "'", this.ambiente);
            resultado = servicio.antiguedadActivar(
                    id_usuario,
                    this.selectedAntiguedad.getAntiguedad(), 
                    this.ambiente);
            this.constructor();
    ...
    public Antiguedad_List getSelectedAntiguedad() {
        return selectedAntiguedad;
    }

    public void setSelectedAntiguedad(Antiguedad_List selectedAntiguedad) {
        this.selectedAntiguedad = selectedAntiguedad;
    }

    public List<Antiguedad_List> getLst_antiguedad() {
        return lst_antiguedad;
    }

    public void setLst_antiguedad(List<Antiguedad_List> lst_antiguedad) {
