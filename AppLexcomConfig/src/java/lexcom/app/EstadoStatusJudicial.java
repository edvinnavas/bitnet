...
@ManagedBean(name = "EstadoStatusJudicial")
@ViewScoped
public class EstadoStatusJudicial implements Serializable {
    ...
    public void init() {
        try {
            ...
            String lista_estado_sql = "select s.sestado, s.nombre from sestado s where s.estado='VIGENTE'";
    ...
    public void cargar_status() {
        try {
            ...
            String cadenasql = "select "
                    ...
                    + "where "
                    + "e.estatus not in "
                    + "(select es.estatus from estado_status_judicial es where es.sestado=" + this.estado_d + ")";
            ...
            cadenasql = "select "
                    + "e.nombre "
                    + "from "
                    + "estado_status_judicial es "
                    + "left join estatus e on (es.estatus=e.estatus) "
                    + "where "
                    + "es.sestado=" + this.estado_d;
    ...
    public void guardar() {
        ...
        try {
            ...
            for(Integer i=0; i<this.status_d.getTarget().size(); i++) {
                lst_status.add(driver.getInt("select e.estatus from estatus e where e.nombre='" + this.status_d.getTarget().get(i) + "'", this.ambiente));
            }
            
            Servicio servicio = new Servicio();
            Integer id_usuario = driver.getInt("select u.usuario from usuario u where u.nombre = '" + this.usuario + "'", this.ambiente);
            resultado = servicio.estadoStatusJudicial(id_usuario, this.estado_d, lst_status, this.ambiente);
            this.constructor();
