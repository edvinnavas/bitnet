...
@ManagedBean(name = "EstatusExtra")
@ViewScoped
public class EstatusExtra implements Serializable {
    ...
    private EstatusExtra_List selectedEstatus;
    private List<EstatusExtra_List> lst_estatus;
    ...
    public void init() {
        try {
            ...
            lst_estatus = new ArrayList<>();
            for(Integer i = 1; i < filas; i++) {
                EstatusExtra_List cor = new EstatusExtra_List(
    ...
    public void carga_info_modificar() {
        if(this.selectedEstatus != null) {
            String cadenasql = "select "
                    ...
                    + "estatus_extra e "
                    + "where "
                    + "e.estatus_extra = " + this.selectedEstatus.getEstatus();
            Servicio servicio = new Servicio();
            java.util.List<lexcom.ws.StringArray> resultado = servicio.reporte(cadenasql, this.ambiente);
            Integer filas = resultado.size();
            Integer columnas = resultado.get(0).getItem().size();
            ...
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Mensaje del sistema...", "Dede seleccionar un status del listado."));
        }
    }
    ...
    private void insertar() {
        ...
        try {
            if (!this.nombre_d.equals("")) {
                ...
                resultado = servicio.statusExtraInsertar(
                        id_usuario,
                        this.nombre_d,
                        this.descripcion_d, 
                        this.ambiente);
                this.constructor();
                RequestContext.getCurrentInstance().execute("PF('dtblWidgetSta').clearFilters();");

                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Mensaje del sistema...", resultado));
            } else {
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Mensaje del sistema...", "Debe ingresar el nombre del status."));
            }
        } catch (Exception ex) {
    ...
    private void modificar() {
        ...
        try {
            if (!this.nombre_d.equals("")) {
                ...
                Servicio servicio = new Servicio();
                Integer id_usuario = driver.getInt("select u.usuario from usuario u where u.nombre = '" + this.usuario + "'", this.ambiente);
                resultado = servicio.statusExtraModificar(
                        id_usuario,
                        this.selectedEstatus .getEstatus(),
                        this.nombre_d,
                        this.descripcion_d, 
                        this.ambiente);
                this.constructor();
    ...
    private void eliminar() {
        ...
        try {
            ...
            Servicio servicio = new Servicio();
            Integer id_usuario = driver.getInt("select u.usuario from usuario u where u.nombre = '" + this.usuario + "'", this.ambiente);
            resultado = servicio.statusExtraEliminar(
                    id_usuario,
                    this.selectedEstatus.getEstatus(), 
                    this.ambiente);
            this.constructor();
    ...
    private void activar() {
        ...
        try {
            ...
            Servicio servicio = new Servicio();
            Integer id_usuario = driver.getInt("select u.usuario from usuario u where u.nombre = '" + this.usuario + "'", this.ambiente);
            resultado = servicio.statusExtraActivar(
                    id_usuario,
                    this.selectedEstatus.getEstatus(), 
                    this.ambiente);
            this.constructor();
    ...
    public EstatusExtra_List getSelectedEstatus() {
        return selectedEstatus;
    }

    public void setSelectedEstatus(EstatusExtra_List selectedEstatus) {
        this.selectedEstatus = selectedEstatus;
    }

    public List<EstatusExtra_List> getLst_estatus() {
        return lst_estatus;
    }

    public void setLst_estatus(List<EstatusExtra_List> lst_estatus) {
        this.lst_estatus = lst_estatus;
    }
