...
public class Monitor implements Serializable {
    ...
    public void obtener_monitor() {
        try {
            ...
            java.util.List<lexcom.ws.StringArray> resultado = servicio.getMonitor("TEST");
            ...
            for (Integer i = 1; i < filas; i++) {
                Date fecha_produccion = formatoFecha_db.parse(vector_result[i][0]);
                this.fecha_ultima_gestion_produccion = formatoFecha_form.format(fecha_produccion);
                this.hora_ultima_gestion_produccion = vector_result[i][1];
                this.numero_gestion_produccion = Integer.parseInt(vector_result[i][2]);
                
                Date fecha_pruebas = formatoFecha_db.parse(vector_result[i][3]);
                this.fecha_ultima_gestion_test = formatoFecha_form.format(fecha_pruebas);
                this.hora_ultima_gestion_test = vector_result[i][4];
                this.numero_gestion_test = Integer.parseInt(vector_result[i][5]);
                
                Date fecha_replica = formatoFecha_db.parse(vector_result[i][6]);
                this.fecha_ultima_gestion_replica = formatoFecha_form.format(fecha_replica);
                this.hora_ultima_gestion_replica = vector_result[i][7];
                this.numero_gestion_replica = Integer.parseInt(vector_result[i][8]);
            }
