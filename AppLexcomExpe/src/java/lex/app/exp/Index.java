...
public class Index implements Serializable {
    ...
    public void validar_reinicio_usuario() {
        try {
            ...
            System.out.println("ERROR => LexcomExpediente-Index(validar_reinicio_usuario): " + ex.toString());
    ...
    public String login() {
        ...
        try {
            ...
            switch (resultado) {
                case "menu": {
                    resultado = "expediente_busqueda";
                    HttpSession session = (HttpSession) FacesContext.getCurrentInstance().getExternalContext().getSession(true);
                    session.setAttribute("id_usuario", this.usuario);
                    session.setAttribute("ambiente", this.ambiente);
                    
                    FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Mensaje del sistema...", "Bienvenido " + this.usuario + " al sistema Lexcom Expedientes."));
                    break;
                }
                case "reinicio": {
            ...
            System.out.println("ERROR => LexcomExpediente-Index(login): " + ex.toString());
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Mensaje del sistema...", ex.toString()));
        }
    ...
    public String loginCambio() {
        ...
        try {
            if (this.Nes_contrasena.equals(this.contrasena)) {
                resultado = "index";
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Mensaje del sistema...", "La contraseña nueva y la antigua deben ser diferentes."));
            } else {
                Servicio servicio = new Servicio();
                ...
                switch (resultado) {
                    case "menu":
                        ...
                        break;
                    case "Ingrese contraeña antigua":
                        this.CommonRender = "false";
                        this.newRender = "true";
            ...
            System.out.println("ERROR => LexcomExpediente-Index(loginCambio): " + ex.toString());
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Mensaje del sistema...", ex.toString()));
        }
    ...
    public void cambiar_ambiente() {
        try {
            ...
            System.out.println("ERROR => LexcomExpediente-Index(cambiar_ambiente): " + ex.toString());
